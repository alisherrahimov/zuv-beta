...
import {getPercelsAction, postAdsPercelAction} from '../redux/apis/percels';
import {getCouriersAction, postAdsCourierAction} from '../redux/apis/courier';
import DatePicker from 'react-native-date-picker';
import {dateToString, dateToTime} from '../components/Modals/FilterModal';

import {LanguageContext} from '../context/LanguageContext';
import {Toast} from 'react-native-toast-message/lib/src/Toast';
import {getToken} from '../helpers/dataStorage';
...
let schemaKuryer = yup.object().shape({
  from_region: yup.string().required(),
  to_region: yup.string().required(),
  from_city: yup.string().required(),
  to_city: yup.string().required(),
...
const AddScreen = () => {
  ...
  const [value, setValue] = useState<PostDataInterface>({
    ...
    type: Number(type) === 1 ? 'courier' : 'parcel',
    from_city: '',
    to_city: '',
  });
  ...
  const [transportType, setTransportType] = useState<number>(0);
  const [loading, setLoading] = useState<boolean>(false);
  const [time, setTime] = useState('');
  const [countError, setCountError] = useState(false);

  const onPostAllInfo = useCallback(() => {
    setLoading(true);
    if (Number(type) == 1) {
      dispatch(postAdsCourierAction(value))
        .then(res => {
          if (res.payload?.success) {
            ...
            dispatch(getCouriersAction()).then(() => {
      ...
      dispatch(postAdsPercelAction(value))
        .then(res => {
          if (res.payload?.success) {
            Toast.show({
              type: 'tomatoToast',
            ...
            dispatch(getPercelsAction()).then(() => {
              navigation.navigate('BottomNavigator');
            });
          } else {
            Toast.show({
  ...
  }, [dispatch, navigation, t, type, value]);
    ...
    try {
      if (Number(type) === 1) {
        if (value.transport_type === 'car') {
          if (
            (value.luggage_type === 'people' && value.people_count === 0) ||
            (value.luggage_type === 'both' && value.people_count === 0)
          ) {
            setCountError(true);
            return;
          } else {
          ...
          await schemaKuryer.validate({...value}, {abortEarly: false});
          onPostAllInfo();
        }
      } else {
        if (value.luggage_type === 'people' && value.people_count === 0) {
          setCountError(true);
          return;
        } else {
          setCountError(false);
  ...
  console.log(countError, 'asdasds');
  console.log(JSON.stringify(errors, null, 2), 'error');
  const renderCargoInput = useMemo(() => {
    return (
      <MyInput
        style={[{flex: 1}, styles.input(errors.comment)]}
          ...
          onPress={() => {
            if (Number(type) === 1) {
                  ...
                  if (pr.second && pr.first === false) {
                    ...
                    if (pr.second) {
                      setValue({
                        ...value,
                        luggage_type: 'people',
                        transport_type: Number(type) === 1 ? 'car' : '',
                      });
                    } else {
              ...
              setValue(val => {
                return {
                  ...val,
                  luggage_type: val.luggage_type === 'parcel' ? '' : 'parcel',
                };
              });
            }
          }}
            ...
            if (Number(type) === 1) {
              ...
              setLocal_Luggage_Type(pr => {
                setValue(val => {
                  return {
                    ...val,
                    luggage_type: val.luggage_type === 'people' ? '' : 'people',
                  };
                });
                return {
                  second: !pr.second,
  ...
  return (
    <View style={styles.container}>
      <OtherHeader title={Number(type) !== 1 ? t('titleYo') : t('titleKu')} />
      <View style={styles.main}>
        <ScrollView
          ...
          <View>
            ...
            <View>
              {value.luggage_type === 'people' ||
              value.luggage_type === 'both' ? (
                <>
                  <CustomText
                    lebel={
    ...
    <View>
      ...
      <View style={styles.transport}>
        <SelectButton
          ...
          active={transport_type === 0 ? true : false}
          image={image.car}
          title={t('car')}
          width="49%"
        />
        {value.luggage_type === 'parcel' ? (
          <>
            <SelectButton
              onPress={() => {
                setTransportType(1);
                setValue({...value, transport_type: 'train'});
              }}
              active={transport_type === 1 ? true : false}
              image={image.train}
              title={t('train')}
              width="49%"
            />
            <SelectButton
              onPress={() => {
                setTransportType(2);
                setValue({...value, transport_type: 'plain'});
              }}
              active={transport_type === 2 ? true : false}
              image={image.air}
              title={t('airplane')}
              width="49%"
              mnTop={12}
